...
class CoinViewThread(QThread): # 시그널 클래스
    ...
    def __init__(self, ticker):
        # MainWindow에서 시그널 클래스로 객체를 선언할 때 ticker를 인수로 전달
        super().__init__()
        self.ticker = ticker
        self.alive = True
    ...
    def run(self):
        while self.alive:
            ...
            param = {"markets": f"KRW-{self.ticker}"}  # 비트코인
            ...
            time.sleep(1) # api호출 딜레이(1초 마다 한번씩 업비트 호출)
    def close(self): # close 함수가 호출되면 run 함수(while 문)이 멈춤
        self.alive = False
...
class MainWindow(QMainWindow, form_class): # 슬롯 클래스
    ...
    def __init__(self, ticker = "BTC"):
        ...
        self.cvt.coinDataSent.connect(self.alarmCheck)
        self.cvt.start() # 시그널 함수의 시작
        self.coin_comboBox_setting() # 콤보박스 초기화 셋팅 함수 호출
        self.alarmButton.clicked.connect(self.alarmButtonAction)
        # 알람버튼에 클릭되면 alarmButtonAction 실행
    ...
    def coin_comboBox_setting(self):
        ...
        coinTickerList = sorted(coinTickerList) # BTC 제거한 상ㅌ태의 리스트를 오름차순으로 정렬
        ...
        # 콤보벅스의 메뉴 값이 변경되었을 때 호출될 함수 설정

    def coin_select_comboBox(self):
        coin_ticker = self.coin_comboBox.currentText() # 콤보박스에서 햔재 선택된 ticker 켁스트 가져오기
        #print(coin_ticker)
    ...
    def telegram(self, message_text):
        telegram_call = TelegramBotClass(self)
        telegram_call.telegramBot()
...
class TelegramBotClass(QThread): # 텔레그램 메시지 봇 클래스
    def __init__(self, parent):
        ...
        self.parent = parent
        token = "552316356:AAGXD8WHQngXA6VPC4NcLnJXncrHXnXRsJY"
        # chat_id = "55*******"
        self.bot = telegram.Bot(token=token)

    def telegramBot(self, text):
        # 텔레그램으로 보낼 메시지를 인수로 넣어서 호출하면 해당 텔레그램 계정으로 전송하는 메서드
        asyncio.run(self.bot.sendMessage(chat_id=552316356, text=text)
